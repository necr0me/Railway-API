RSpec.describe TicketPolicy, type: :policy do
  ...
  let(:record) { create(:ticket, user: user) }
  ...
  context "when user role is :user" do
    ...
    context "when user is incorrect (user is not owner of ticket)" do
      let(:record) { create(:ticket, user: create(:user, email: "m@mail.com")) } # TODO: remove email after factory fix
  ...
  context "when user role is :moderator" do
    ...
    context "when user is incorrect (user is not owner of ticket)" do
      let(:record) { create(:ticket, user: create(:user, email: "m@mail.com")) }
